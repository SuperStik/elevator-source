...
if SERVER then
	...
	function ENT:Use(ply)
		...
		if not self:IsWatching(ply) then
			self:TurnOnTV(ply)
		else
			self:TurnOffTV(ply)
		end
	...
	function ENT:IsWatching(ply)
		return IsValid(ply.ActiveTV) and ply.ActiveTV == self
	end
	...
	function ENT:UpdateAllPlayers(starter)
		for _, ply in pairs(player.GetAll()) do
			...
			if IsValid(starter) and ply == starter then continue end
	...
	concommand.Add("elev_settv", function(ply, cmd, args)
		...
		if not tvindex then return end
		local tv = Entity(tvindex)
		if not IsValid(tv) then return end
		tv:SetTV(ply, vid)
	end)
else -- Client
	...
	function ENT:TurnOn()
		...
		if not IsValid(self.Browser) then
	...
	function ENT:DisplayControls()
		...
		if not IsValid(self.HTMLFrame) then
	...
	function ENT:Think()
		...
		if self.On and LocalPlayer():GetPos():Distance(self:GetPos()) > 300 then
			self:TurnOff()
		end
	end
	...
	usermessage.Hook("elevator_tvvolume", function(um)
		...
		if not LocalPlayer().TVVolume then
...
if CLIENT then
	...
	function PANEL:Init()
		...
		self:AddFunction("gmt", "VideoID", function(id)
			self:SelectedVideo(id)
		end)
	end
	...
	function PANEL:SelectedVideo(id)
		if IsValid(self.Entity) and id ~= self.Entity.VideoID then
			-- Update clients
		...
		-- Close starter's TV
		if IsValid(self.Entity.HTMLFrame) then
